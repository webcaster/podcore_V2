...
Der Tutorialstart ist gegen einen React-Hook-Reihenfolgefehler korrigiert, der beim Wechsel von keiner aktiven Führung zu einem geöffneten Tutorial die gesamte Ansicht leeren konnte. Eine lokale Fehlergrenze stellt sicher, dass ein Fehler der Tutorialkarte nie mehr die übrige App blockiert. Stattdessen bleibt eine bedienbare Fallback-Hilfe mit Schließen- und Wiki-Aktion sichtbar.

Mehrfach-Podcast ist jetzt bewusst optional. Unter **Einstellungen → Podcast** kann die Funktion aktiviert werden. Danach öffnet **Profile wechseln** die Podcastverwaltung; über **Aktiv setzen** wird das zweite angelegte Profil ausgewählt und der Arbeitsbereich mit dessen getrenntem Datenbereich neu geladen.

Im Admin-Tab **Logs** können Administratoren Protokolle als JSON exportieren. Eine Bereinigung verlangt die Texteingabe **„LOGS LÖSCHEN“** und löscht ausschließlich Einträge der Fehlerprotokolltabelle – keine Episoden, Medien, Backups oder anderen Anwendungsdaten.
...
Der Episodeneditor besitzt einen neuen Tab **„Qualitätsgate“**. Dort werden Aufnahmebereitschaft, Studio- und Setuphinweise, Audio-Abnahme, Release-Paket, Rechte-Register sowie externe Übergaben direkt mit der Episode gespeichert. Für die Zusammenarbeit lassen sich strukturierte JSON-Übergabepakete und kopierfertige Markdown-Release-Pakete exportieren. Die neuen Daten erweitern die vorhandenen technischen Episodendaten ohne Datenbankmigration und bleiben dadurch auch in bestehenden Sicherungs- und Archivabläufen verfügbar.

### Neu in v2.16.36: Sichtbare Tutorialkarte über jeder App-Ansicht

Die interaktive Tutorialkarte wird direkt im Dokumentkörper gerendert und ist damit unabhängig von Layout-, Transformations- und Ebenenkontexten einzelner PodCore-Seiten. Der abgedunkelte Hintergrund kann nicht mehr ohne sichtbares Tutorialfenster erscheinen. Die Karte, der Hintergrund und der hervorgehobene Zielbereich verwenden getrennte feste Ebenen; die Sichtbarkeit wurde auf Desktop und mobilen Breiten mit einem isolierten Render-Test geprüft.

### Neu in v2.16.35: Add-on für mehrere Podcasts

Die neue **Podcast-Verwaltung** ermöglicht berechtigten Nutzern, mehrere Podcast-Profile in einer PodCore-Installation anzulegen, zu bearbeiten, zu löschen und eines davon als aktives Profil zu setzen. Gespeichert werden Name, Untertitel, Beschreibung, Host, Kategorie, Sprache, Website, RSS-Feed und Akzentfarbe. Die Auswahl bleibt über Sitzungen hinweg erhalten. Details und der technische Umfang stehen in [`docs/RELEASE-2.16.35.md`](docs/RELEASE-2.16.35.md).
